...
const exchangeRecommendInfo = (state = {
    filter: {
        type: ''
    },
    search: {
    ...
    switch (action.type) {
        case EXCHANGERECOMMEND.ADD_DATA:
            return {...state, ...action.data}
        case EXCHANGERECOMMEND.ADD_QUERY:
            return {...state, query: {..._query, ...action.data}}
        case EXCHANGERECOMMEND.SET_SEARCH_QUERY:
            return {...state, search: {...search, ...action.data}}
        case EXCHANGERECOMMEND.SET_PAGE_DATA:
            return {...state, pageData: {...pageData, ...action.data}}
        case EXCHANGERECOMMEND.SET_FILTER_DATA:
            return {...state, filter: {...filter, ...action.data}}
        case EXCHANGERECOMMEND.EDIT_USER_INFO:
            return {...state, userInfo: {..._userInfo, ...action.data}}
        case EXCHANGERECOMMEND.EDIT_LIST_ITEM:
        ...
        case EXCHANGERECOMMEND.DEL_LIST_ITEM:
            return {...state, list: [..._list.slice(0, action.index), ..._list.slice(action.index + 1)]}
        case EXCHANGERECOMMEND.DEL_REPLY_LIST:
...
export default exchangeRecommendInfo

...
import './webCoinRecommend.scss'
import IconItem from '../../../components/icon/icon'
import CollectionCreateForm from './ModalForm'
import {getWebCoinRecommendList, setSearchQuery, setPageData, addCoin, selectData} from '../../../actions/entries/webCoinRecommend.action'
import {axiosAjax, cutString, formatDate} from '../../../public/index'
const confirm = Modal.confirm
let columns = []
class WebCoinRecommendIndex extends Component {
    constructor () {
        super()
        this.state = {
            loading: true,
    ...
    componentWillMount () {
        ...
        columns = [{
            title: '名称',
            key: 'symbol',
            render: (text, record) => (record && <div className="webCoinRecommend-info clearfix">
                <div>
            ...
            render: (item) => (<div className="btns">
                ...
                <p>
                    <a onClick={() => this.delWebCoinRecommend(item)} className="mr10 opt-btn" href="javascript:void(0)" style={{background: '#d73435'}}>删除</a>
                </p>
                {/*
                <p>
                    <a
    ...
    doSearch (type, data) {
        ...
        dispatch(getWebCoinRecommendList(type, sendData, () => {
            this.setState({
                loading: false
            })
        }))
    ...
    delWebCoinRecommend (item) {
        ...
        confirm({
            ...
            onOk () {
                ...
                axiosAjax('POST', '/coin/webrecommend/del', {...sendData}, (res) => {
                    if (res.code === 1) {
    ...
    submitAccount = () => {
        ...
        form.validateFields((err, values) => {
            ...
            if (selectedData.id) {
                axiosAjax('post', '/coin/webrecommend/update', {
                    id: selectedData.id,
                    ...sendData
                }, (data) => {
                    if (data.code === 1) {
    ...
    render () {
        ...
        return <div className="webCoinRecommend-index">
            <Row>
                <Col>
...
const mapStateToProps = (state) => {
    return {
        webCoinRecommendInfo: state.webCoinRecommendInfo,
        list: state.webCoinRecommendInfo.list,
        search: state.webCoinRecommendInfo.search,
        filter: state.webCoinRecommendInfo.filter,
        pageData: state.webCoinRecommendInfo.pageData,
        selectedData: state.webCoinRecommendInfo.selectedData
    }
}

export default connect(mapStateToProps)(Form.create()(WebCoinRecommendIndex))

...
import {getCQTeamList, setSearchQuery, setPageData, setFilterData, selectedData} from '../../actions/CQteam/CQteam.action'
...
let columns = []
class CQteamIndex extends Component {
    constructor () {
        super()
        this.state = {
            loading: true,
        ...
        let optionCol = [{
            ...
            render: (item) => {
                return <div className="btns">
                    <p>
                        <Link className="mr10 opt-btn" to={{pathname: '/CQTeam-send', query: {id: item.id, passportId: item.passportId}}} style={{background: '#e35ba3'}}>编辑</Link>
                    </p>
                    <p>
                    ...
                    {(() => {
                        if (item.registrationState === 1) {
                            ...
                            return <p>
                                <Link className="mr10 opt-btn" to={{pathname: '/CQTeam-send', query: {id: item.id, passportId: item.passportId}}} style={{background: '#108ee9'}}>开始审核</Link>
                            </p>
                        }
                    })()}
                    {/*
                    <p>
                        <a onClick={() => this.delCQTeam(item)} className="mr10 opt-btn" href="javascript:void(0)" style={{background: '#d73435'}}>删除</a>
                    </p>
                    */}
                </div>
            }
    ...
    doSearch (type, data) {
        ...
        dispatch(getCQTeamList(type, sendData, () => {
            this.setState({
                loading: false
            })
        }))
...
export default connect(mapStateToProps)(Form.create()(CQteamIndex))

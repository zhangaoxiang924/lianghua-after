...
import { Table, Row, Col, Modal, message, Spin, Select, Input, Form, DatePicker, Tooltip } from 'antd'
import './index.scss'
import { Link } from 'react-router'
// import IconItem from '../../components/icon/icon'
import {getMiniAppList, setSearchQuery, setPageData, setFilterData, newsToTop, editMiniAppList, selectedData} from '../../../actions/miniApp/miniApp.action'
...
let columns = []
class MiniAppIndex extends Component {
    constructor () {
        super()
        this.state = {
            loading: true,
    ...
    componentWillMount () {
        ...
        let optionCol = [
            ...
            //         </p>
            //         <p>
            //             {item.topOrder && parseInt(item.topOrder) ? <Tooltip placement="bottom" title={`置顶失效时间: ${moment(item.topEndTime).format('YYYY年MM月DD日 HH:mm:ss')}; 失效热度: ${item.topEndHotcount}`} >
            //                 <a
            ...
            // },
            {
                ...
                render: (item) => {
                    return <div className="btns">
                        <p>
                            <Link className="mr10 opt-btn" to={{pathname: '/miniNews-detail', query: {id: item.id}}} style={{background: '#108ee9'}}>详情</Link>
                        </p>
                        {/*
                        <p>
                            <Link className="mr10 opt-btn" to={{pathname: '/miniNews-send', query: {id: item.id}}} style={{background: '#e35ba3'}}>编辑</Link>
                        </p>
                        */}
                        <p>
                        ...
                        <p>
                            <a onClick={() => this.delMiniApp(item)} className="mr10 opt-btn" href="javascript:void(0)" style={{background: '#d73435'}}>删除</a>
                        </p>
                    </div>
                }
            }
        ...
        let basicCol = [{
            ...
            width: 230,
            key: 'name',
            render: (text, record) => (record && <div className="miniApp-info clearfix">
                <div>
                    ...
                    <div>
                        ...
                        {!record.topOrder || parseInt(record.topOrder) === 0 ? '' : <Tooltip placement="bottom" title={`失效时间: ${moment(record.topEndTime).format('YYYY年MM月DD日 HH:mm:ss')}; 失效热度: ${record.topEndHotcount}`} >
    ...
    changeTopValue (e, record) {
        ...
        dispatch(editMiniAppList({topOrder: val}, record.key))
    }
    ...
    doSearch (type, data) {
        ...
        dispatch(getMiniAppList(type, sendData, () => {
            this.setState({
                loading: false
            })
        }))
    ...
    render () {
        ...
        return <div className="miniApp-index">
            <Row>
                <Col>
                    <span>新闻状态：</span>
                    ...
                    {/*
                    <Button type="primary" style={{margin: '0 0 0 15px'}} onClick={() => { hashHistory.push('/miniApp-send') }}><IconItem type="icon-post-send"/>新增</Button>
                    */}
                </Col>
                {/*
                <Col>
...
const mapStateToProps = (state) => {
    return {
        miniAppInfo: state.miniAppInfo,
        list: state.miniAppInfo.list,
        search: state.miniAppInfo.search,
        filter: state.miniAppInfo.filter,
        pageData: state.miniAppInfo.pageData,
        selectedData: state.miniAppInfo.selectedData,
        channelList: state.channelListInfo
    }
}

export default connect(mapStateToProps)(Form.create()(MiniAppIndex))
